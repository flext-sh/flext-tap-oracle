...
**Package:** `flext_tap_oracle` · deps: `flext-core`, `flext-db-oracle`,
`flext-meltano`, `flext-observability`

## Overview

Singer **tap** (extractor) for Oracle Database. Thin driver over `flext-meltano`
(ADR-006), delegating connectivity to `flext-db-oracle`.
...
- Discovery is **command-driven** (not a minimal tap class); `rules.json` + `streams.py`
