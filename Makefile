...
discover: ## Run tap discovery mode
	$(POETRY) run tap-oracle --config $(TAP_CONFIG) --discover > $(TAP_CATALOG)

run: ## Run tap extraction
	$(POETRY) run tap-oracle --config $(TAP_CONFIG) --catalog $(TAP_CATALOG) --state $(TAP_STATE)
...
oracle-test: ## Test Oracle connection
	PYTHONPATH=$(SRC_DIR) $(POETRY) run python -c "from flext_tap_oracle.client import test_connection; test_connection()"
...
clean: ## Clean build artifacts
	rm -rf build/ dist/ *.egg-info/ .pytest_cache/ htmlcov/ .coverage .mypy_cache/ .pyrefly_cache/ .ruff_cache/ $(TAP_CATALOG) $(TAP_STATE)
...
diagnose: ## Project diagnostics
	...
	@echo "Singer SDK: $$(PYTHONPATH=$(SRC_DIR) $(POETRY) run python -c 'import singer_sdk; print(singer_sdk.__version__)')"
	@$(POETRY) env info
...
.DEFAULT_GOAL := help
.PHONY: help install install-dev setup validate check lint format type-check security fix test test-unit test-integration test-singer test-fast coverage-html discover run validate-config catalog sync oracle-test build build-clean docs docs-serve deps-update deps-show deps-audit shell pre-commit clean clean-all reset diagnose doctor t l f tc c i v d r# =============================================================================
# MAINTENANCE
# =============================================================================
